// main_enhanced.cpp - Enhanced DSS Image Matcher with FITS Loading, WCS Matching, and Caching
#include "DSSMatcher.h"
#include "MessierCatalog.h"
#include "FitsProcessor.h"
#include "ImageCache.h"
#include "ImageMatcherDialog.h"
...
class DSSViewerWindow : public QMainWindow {
    Q_OBJECT

private:
    DSSImageMatcher* matcher;
    ImageCache* cache;
    FitsProcessor* fitsProcessor;
...
public:
    DSSViewerWindow(QWidget* parent = nullptr) : QMainWindow(parent), 
        compositeFetchCount(0), fetchingComposite(false) {
        setWindowTitle("DSS Image Matcher - Enhanced with WCS & Analysis");
        resize(1400, 900);
        
        // Create components
        matcher = new DSSImageMatcher(this);
        cache = new ImageCache(this);
        fitsProcessor = new FitsProcessor(this);
...
private:
    void setupMenuBar() {
        ...
        QAction* loadFitsAction = new QAction("&Load User FITS...", this);
        loadFitsAction->setShortcut(QKeySequence::Open);
        connect(loadFitsAction, &QAction::triggered, this, &DSSViewerWindow::onLoadUserFits);
        fileMenu->addAction(loadFitsAction);
        ...
        QAction* cacheInfoAction = new QAction("Cache &Info...", this);
        connect(cacheInfoAction, &QAction::triggered, this, &DSSViewerWindow::showCacheInfo);
        cacheMenu->addAction(cacheInfoAction);
        
        QAction* clearCacheAction = new QAction("&Clear Cache...", this);
        connect(clearCacheAction, &QAction::triggered, this, &DSSViewerWindow::onClearCache);
        cacheMenu->addAction(clearCacheAction);
        
        QAction* cleanupAction = new QAction("Cleanup &Old Entries...", this);
        connect(cleanupAction, &QAction::triggered, this, &DSSViewerWindow::onCleanupCache);
        cacheMenu->addAction(cleanupAction);
    }
    ...
    void setupUI() {
        ...
        // Messier object selection (same as before)
        ...
        messierObjectList->setMaximumHeight(250);
        ...
        objectInfoLabel->setMinimumHeight(120);
        ...
        QLabel* formatInfo = new QLabel("Format: FITS (cached automatically)");
        ...
        // NEW: User FITS controls
        QGroupBox* userFitsGroup = new QGroupBox("User FITS Analysis");
        QVBoxLayout* userFitsLayout = new QVBoxLayout(userFitsGroup);
        
        loadUserFitsBtn = new QPushButton("Load Your FITS File...");
        loadUserFitsBtn->setStyleSheet("QPushButton { padding: 8px; background-color: #2196F3; color: white; }");
        userFitsLayout->addWidget(loadUserFitsBtn);
        
        matchImagesBtn = new QPushButton("Match & Analyze with DSS Image");
        matchImagesBtn->setEnabled(false);
        matchImagesBtn->setStyleSheet("QPushButton { padding: 8px; background-color: #FF9800; color: white; }");
        userFitsLayout->addWidget(matchImagesBtn);
        
        leftPanel->addWidget(userFitsGroup);
    ...
    void connectSignals() {
        ...
        connect(loadUserFitsBtn, &QPushButton::clicked, this, &DSSViewerWindow::onLoadUserFits);
        connect(matchImagesBtn, &QPushButton::clicked, this, &DSSViewerWindow::onMatchImages);
        ...
        connect(matcher, &DSSImageMatcher::imageReceived, this, &DSSViewerWindow::onImageReceived);
        connect(matcher, &DSSImageMatcher::fitsDataReceived, this, &DSSViewerWindow::onFitsReceived);
        connect(matcher, &DSSImageMatcher::errorOccurred, this, &DSSViewerWindow::onError);
    }
...
private slots:
    ...
    void displayObjectInfo(const MessierObject& obj) {
        ...
        info += QString("<b>Imaged:</b> %1").arg(obj.has_been_imaged ? "Yes ✓" : "No");
    ...
    void onLoadUserFits() {
        ...
        if (!fileName.isEmpty()) {
            userFitsPath = fileName;
            statusLabel->setText("User FITS loaded: " + QFileInfo(fileName).fileName());
            statusLabel->setStyleSheet("QLabel { padding: 5px; background-color: #d4edda; }");
            matchImagesBtn->setEnabled(!currentImageData.isEmpty());
        }
    }
    
    void onMatchImages() {
        if (userFitsPath.isEmpty()) {
            QMessageBox::warning(this, "No User FITS", "Please load a FITS file first!");
            return;
        }
        
        if (currentImageData.isEmpty()) {
            QMessageBox::warning(this, "No Library Image", 
                               "Please fetch a DSS library image first!");
            return;
        }
        
        ImageMatcherDialog* dialog = new ImageMatcherDialog(
            userFitsPath, currentImageData, this);
        dialog->exec();
    }
    ...
    void showCacheInfo() {
        ...
        QMessageBox::information(this, "Cache Information", info);
    }
    
    void onClearCache() {
        auto reply = QMessageBox::question(this, "Clear Cache",
    ...
    void onCleanupCache() {
        cache->cleanupOldEntries(720); // 30 days
        QMessageBox::information(this, "Cache Cleanup", 
                                "Old cache entries (>30 days) have been removed.");
    }
    ...
    void onSaveImage() {
        ...
        if (!fileName.isEmpty()) {
            ...
            if (isComposite) {
                ...
                // Save raw FITS data
                success = matcher->saveImage(currentImageData, fileName);
            }
    ...
    void loadCompositeFits(const QByteArray& fitsData) {
        // Parse composite FITS and display
        currentImage = parseFitsToImage(fitsData);
        currentImageData = fitsData;
        ...
        if (!currentImage.isNull()) {
            ...
            statusLabel->setText(QString("Composite loaded from cache: %1×%2")
                                .arg(currentImage.width())
                                .arg(currentImage.height()));
            statusLabel->setStyleSheet("QLabel { padding: 5px; background-color: #d4edda; }");
        }
...
int main(int argc, char *argv[]) {
    ...
    app.setApplicationName("DSS Image Matcher");
