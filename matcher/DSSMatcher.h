...
public:
    explicit DSSImageMatcher(QObject* parent = nullptr) 
        : QObject(parent), 
...
int main(int argc, char *argv[]) {
    ...
    // Create matcher
    DSSImageMatcher* matcher = new DSSImageMatcher();
    ...
    QObject::connect(matcher, &DSSImageMatcher::imageReceived, 
                    [imageLabel](const QImage& img, const QByteArray& raw) {
        imageLabel->setPixmap(QPixmap::fromImage(img));
        qDebug() << "Image displayed!";
    });
    
    QObject::connect(matcher, &DSSImageMatcher::errorOccurred,
                    [](const QString& error) {
        qDebug() << "Error:" << error;
    });
    
    // Fetch M51 (Whirlpool Galaxy)
    matcher->fetchByObjectName("M51", 20.0, 20.0);
    
    // Or fetch by coordinates (M42 - Orion Nebula)
    // matcher->fetchByCoordinates(83.8221, -5.3911, 30.0, 30.0);
